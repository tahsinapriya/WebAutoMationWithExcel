...
import com.tahsina.utilities.ExcelUtils;
import com.tahsina.utilities.GetScreenshot;
import com.tahsina.utilities.JsonData;
...
public class LoginPage {
	...
	public void login() throws InterruptedException, IOException {
		//excelData.ReadExcel();
		JsonData jsonfilereader =new JsonData();
		

		try {
			test.info("Please enter username");
			if (username.isDisplayed()) {
				username.sendKeys(jsonfilereader.getString("username"));
				passCase("Username entered");

				try {
					test.info("Enter password");
					if (password.isDisplayed()) {
						password.sendKeys(jsonfilereader.getString("password"));
